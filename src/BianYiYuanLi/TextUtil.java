...
public class TextUtil {
    ...
    public static boolean containsbA(TreeSet<Character> nvSet, String itemCharStr, Character a,
        ...
        String aStr = a.toString();//将a变成字符串
...
//            System.out.println("整合后的句子：" + expressionMap.toString());
            ArrayList<String> arrayList = expressionMap.get(alastChar);
            if (arrayList.contains("ε")) {
//                System.out.println(alastChar + " 可以推出空串且在" + aStr+"后");
                return true;
            }
        }
        return false;
    ...
    public static boolean containsAb(TreeSet<Character> ntSet, String itemCharStr, Character a) {
        String aStr = a.toString();
        if (itemCharStr.contains(aStr)) {//如果后面的表达式包含该非终结符的话
            int aIndex = itemCharStr.indexOf(aStr);//找出了下标
            String findStr;
            try {
                findStr = itemCharStr.substring(aIndex + 1, aIndex + 2);//截取该非终结符的后一个
            } catch (Exception e) {
                return false;
            }
            if (ntSet.contains(findStr.charAt(0))) {//如果是终结符的话
                return true;
            } else {
                return false;
            }
